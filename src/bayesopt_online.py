...
class Bayesopt(object):
    ...
    def __init__(self, password, passphrase, workdir, maxbo=15, initbo=5, x=0.0, y=0.0, MAPI_KEY='4oBTKz0pkFSg9EUQ'):
        ...
        with open("{0}/testoutput.txt".format(str(self.localpath)), "a+") as file:
            ...
            with open(self.localpath+'/slab', 'r') as slab:
                lines = slab.readlines()
                for line in lines:
                    file.write(line)
            file.write('Molecule\n')
            with open(self.localpath+'/molecule', 'r') as slab:
                lines = slab.readlines()
                for line in lines:
                    file.write(line)
            file.write('Domain\n')

            with open(self.localpath+'/domain', 'r') as slab:
                lines = slab.readlines()
            ...
            ase.io.write(self.localpath+'/structure', self.structure, format='vasp')
            ase.io.write(self.localpath+'/vasp/POSCAR', self.structure, format='vasp')
            ...
            with open(self.localpath+'/vasp/submit_file.sh', 'r') as jobfile:
                self.submitfile = jobfile.read()

            file.write('Start BO')
            self.runopt()

            outf = open("../asapdata/{0}/output.txt".format(str(self.workdir)), "a+")
            outf.write('Finished')
            outf.close()
            file.write('Finished\n')
    ...
    def calculate(self, x):
        ...
        with open("../asapdata/{0}/log.txt".format(str(self.workdir)), "a+") as file:
            ...
            import shutil
            shutil.copy2("../asapdata/{0}/vasp/KPOINTS".format(str(self.workdir)), self.step_path)  # complete target filename given
            shutil.copy2("../asapdata/{0}/vasp/INCAR".format(str(self.workdir)), self.step_path)  # complete target filename given
            shutil.copy2("../asapdata/{0}/vasp/POTCAR".format(str(self.workdir)), self.step_path)  # complete target filename given
            shutil.copy2("../asapdata/{0}/vasp/job_submit".format(str(self.workdir)), self.step_path)  # complete target filename given
...
def main(argv):
    ...
    opts, args = getopt.getopt(argv, ":", ["password=", "passphrase=", "workdir=", "maxbo=", "initbo=", "x=", "y="])
