...
class NGLDisplay:
    ...
    def __init__(self, atoms, xsize=500, ysize=500):
        ...
        from ipywidgets import Dropdown, FloatSlider, IntSlider, HBox, VBox, RadioButtons
        ...
        self.vsel = RadioButtons(options=['ball_and_stick', 'atoms only'],
        ...
        self.vsel.observe(self._update_view)
        ...
        wdg = [self.vsel, self.asel, self.csel, self.rad]
        if self.frm:
            wdg.append(self.frm)
    ...
    def _update_view(self, chg=None):
        if self.vsel.value == 'atoms only':
            self.view.remove_ball_and_stick()
            #self.view.add_spacefill()
        else:
